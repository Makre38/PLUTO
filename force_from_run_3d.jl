...
function compute_force_x(rho::Array{Float64, 3}, x::AbstractVector, y::AbstractVector, z::AbstractVector,
                         dx::AbstractVector, dy::AbstractVector, dz::AbstractVector;
                         mp::Float64, rho0::Float64, xp::Float64, yp::Float64, zp::Float64, rcut::Float64)
    fx = 0.0
...
function main()
    if isempty(ARGS)
        error("Usage: julia force_from_run_3d.jl RUN_DIR [TARGET_LOG_LAMBDA]")
    end
    ...
    fx = compute_force_x(rho, x, y, z, dx, dy, dz; mp = mp, rho0 = rho0, xp = xp, yp = yp, zp = zp, rcut = rcut)
